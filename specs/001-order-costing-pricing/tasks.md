...
- Every test task MUST be run and confirmed **failing** before its paired implementation task starts (Constitution Principle I, non-negotiable), except each story's e2e regression task, which is confirmed passing after the story's implementation is done. No implementation task in this list is unpaired.
