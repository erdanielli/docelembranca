...
**Tests**: Test tasks are included and REQUIRED — Constitution Principle I (Test-First Development) is NON-NEGOTIABLE for this project. **Every** implementation task below is preceded by a test task that must be run and confirmed failing first, including navigation, list, and wiring components. **Exception**: Playwright e2e tasks are regression coverage for a story's user-facing flow as a whole, confirmed *passing* once that story's implementation is complete, not a red-green pair confirmed failing first. This is stated outright in Principle I as of constitution v1.6.1 — it is a carve-out the constitution itself grants, not one this file invented (see also research.md §12).

**Not an exception, but read differently**: T117 is a verification *sweep* over guards that earlier phases were already supposed to install. Principle I's red-green rule governs writing implementation code, so an assertion that passes on first run means there is nothing to implement and nothing to redden — it is confirmed coverage. Only an assertion that *fails* implies a missing guard, and that failure is then the red for the migration added in the same task.

**Revision 3 additions (2026-09-16)**: this file was amended, not regenerated, to layer in two constitution changes without disturbing completed work or existing task IDs (some already have GitHub Issues per the Development Workflow). New tasks use a letter suffix anchored to the nearest preceding original task ID (e.g. `T041a`) rather than renumbering anything from `T042` onward — see the Notes section. Two new task kinds appear: (1) backfilled Playwright e2e tasks per story, and (2) a UI-prototype-and-approval checkpoint task (constitution v1.6.0, Principle XII) at the start of each UI-bearing story not yet implemented, which gates that story's implementation tasks on Eduardo's explicit approval of a throwaway `prototypes/` React prototype.

**Revision 4 amendments (2026-09-16)**: a second `/speckit-analyze` pass produced nine changes here, again by amendment rather than regeneration. (1) The e2e exception in the Tests note above now cites constitution v1.6.1, which grants it in Principle I directly — previously this file asserted the exception on its own while citing a research note that required the opposite. (2) `stock_batches.unit_price` is renamed `package_price` throughout (T044, T045, T052, T057, T074) and the mirrored client helper `packageUnitCost` becomes `costPerUnitAmount` (T047, T048): it is the only price in the feature not expressed per canonical unit, and the old name made it indistinguishable from `estimated_unit_price` and `unit_cost_snapshot`, which are. Nothing was implemented against the old names. (3) T077's shared fixtures gain two undated-Batch (`expiration_date IS NULL`) cases, asserted by T074, T078 and T080 — the case where the TypeScript and SQL batch orderings diverge in opposite directions, and the one the fixture set was missing. (4) T095a/T103a add the `awaiting_quote → quoting` client action, which had an RPC edge (T085) and a pgTAP test (T084) but no component to invoke it. (5) `stock_batch_availability.reserved_amount` now excludes reservations whose consumption is already confirmed, so an Order partway through Consolidation stops holding both the deducted amount and its original commitment against the same Batch; asserted from the view side in T074 and from the RPC side in T115. (6) FR-027 now carries the two degenerate allocation cases — labor split evenly when every line's cost is zero, and a zero discount share when the pre-discount subtotal is zero — which previously lived only in `costing-engine.md` while spec.md's Assumptions explicitly disowned any per-line-count basis; T091 asserts both. (7) The FR-037 terminal read-only guard now covers `order_stock_consumptions` (T109, T110, T117), which it had omitted even though those rows are the "actual" half of FR-035's comparison; T117 is also reworded from a self-contradictory "must fail, then confirm it already passes" into an explicit sweep. (8) T113 now asserts that `transition_order_status` actually **persists** the values it is handed — `delivered_at`/`payment_method` (FR-032) and `cancel_reason`/`cancel_impact_note` (FR-036) — since validating a parameter and then dropping it would satisfy every assertion the task previously carried while leaving no record at all. (9) T007 gains the half of research.md §10's devcontainer fix it had omitted: `forwardPorts` must be **removed** when host networking is adopted, or the IDE squats 54322 and `supabase start` fails; quickstart.md's prerequisites, which still described port publishing and an uninitialized stack, are corrected to match.
...
- [x] T007 Add `"runArgs": ["--network=host"]` to `.devcontainer/devcontainer.json` **and remove `forwardPorts`/`portsAttributes` in the same edit** — under a shared network namespace they are not merely redundant: the IDE binds every listed port on the host loopback, so `supabase start` then dies with `address already in use` on 54322, the very port it needs (research.md §10). Rebuild the container, then bring the local stack up with `supabase start`, confirm the API (54321) and Studio (54323) are reachable from the host browser, and document the local-database commands in a "Local database (tests)" section of `README.md`, keeping the hosted `supabase link` + `db push` flow as the deployment path (research.md §10 — without the shared network namespace the CLI health-checks an empty loopback and stops the stack it started)
...
### e2e regression for User Story 1 (backfilled per constitution v1.5.0)

- [X] T041a [P] [US1] Backfill: Playwright e2e spec `tests/e2e/catalog-ingredients.spec.ts` covering Ingredient CRUD and deactivate/reactivate (FR-001/FR-008a), written retroactively against the completed US1 implementation (commit `447a3f2`); authenticates via `dev-preview.html` per research.md §12 — done
- [X] T041b [P] [US1] Backfill: Playwright e2e spec `tests/e2e/catalog-materials.spec.ts` mirroring T041a for Materials (FR-002/FR-008a) (commit `447a3f2`) — done
- [X] T041c [US1] Backfill: Playwright e2e spec `tests/e2e/catalog-recipes.spec.ts` covering Recipe creation with multi-variant Ingredient + Material composition — add, edit, delete (FR-003–FR-007) (commit `447a3f2`) — done

**Checkpoint**: User Story 1 is fully functional and independently testable — a usable recipe book before Stock or Orders exist. US1 predates constitution v1.6.0 (Principle XII, UI prototyping) and is **grandfathered**: it was implemented without a throwaway-prototype-and-approval step, and that is not retroactive. Shared e2e infrastructure this story's backfill relies on — `playwright.config.ts`, `dev-preview.html`/`src/dev-preview.tsx`, `tests/e2e/global-setup.ts`, `tests/e2e/helpers.ts`, `tests/e2e/smoke.spec.ts` — already exists at the repo root from commits `447a3f2`/`d73ccb0` and is not re-listed as per-story tasks here.
...
### UI prototype checkpoint for User Story 2 (constitution v1.6.0, Principle XII)

- [ ] T041d [US2] **Prototype checkpoint — Eduardo approval required.** Build a throwaway React prototype of the Stock/shelf screens (Stock Product linking, Batch entry, Bulk-pack entry, shelf list with remaining amounts) under gitignored `prototypes/stock/`, using mock/static data only — no tests, no Supabase wiring. Present it to Eduardo for explicit approval. **Do not start T042 or any other Phase 4 task until approved.** The prototype is discarded afterward and never merged (research.md §13).
...
- [ ] T044 [P] [US2] Write pgTAP test `supabase/tests/database/111_stock_batches.test.sql` asserting RLS; that `derive_batch_amounts` computes `initial_amount = packages_purchased * convert_unit(package_amount, package_unit, item unit)` (10 × 395 g = 3950 g) and defaults `remaining_amount` to it; that `expiration_date` accepts NULL for a non-perishable Material (FR-011); and the `package_price` > 0, `packages_purchased` > 0, `remaining_amount` ≥ 0 checks — must fail
- [ ] T045 [US2] Run `supabase migration new stock_batches_table` and, in the generated `supabase/migrations/<timestamp>_stock_batches_table.sql`, create `stock_batches` (`id`, `stock_product_id` fk required, `packages_purchased` integer > 0 required, `package_price` numeric > 0 required per package (named for its denominator — it is the one price in the feature that is not per canonical unit, data-model.md), `initial_amount` numeric > 0, `remaining_amount` numeric ≥ 0 default = `initial_amount`, `expiration_date` date **nullable**, `purchase_date`/`purchase_location` nullable, `created_at`, `updated_at`) plus the `derive_batch_amounts` trigger, with RLS (FR-011/FR-012/FR-014); confirm T044 passes
...
- [ ] T047 [P] [US2] Write Vitest unit test `tests/unit/costing/costPerUnitAmount.test.ts` asserting the per-canonical-unit cost of a Batch (`package_price` ÷ package content converted via `convert()`), including a kg-product/g-ingredient case (FR-010) — must fail
- [ ] T048 [US2] Implement `src/lib/costing/costPerUnitAmount.ts` satisfying T047, mirroring the `cost_per_unit_amount` expression the `stock_batch_availability` view will use (T076) — same name as the view column on purpose, so the two sides of the mirror are greppable together
...
- [ ] T052 [P] [US2] Write RTL test `tests/components/stock/BulkPackEntry.test.tsx` asserting that entering a pack size (24) and a total price stores `package_price = total / 24` (FR-013) — must fail
...
- [ ] T057 [US2] Implement `src/features/stock/BulkPackEntry.tsx` satisfying T052, feeding the derived `package_price` into the same Batch-creation path as T055
- [ ] T058 [US2] Implement `src/features/stock/StockTab.tsx` and wire it into the Estoque tab from T019, satisfying T053

### e2e regression for User Story 2 (constitution v1.5.0)

- [ ] T058a [US2] Write Playwright e2e spec `tests/e2e/stock-shelf.spec.ts` covering the shelf flow end to end: link a Stock Product, register two Batches at different prices/expiration dates, register a bulk pack and confirm the derived per-package price, and register a Material batch with no expiration date; run against the completed Phase 4 implementation and confirm passing (constitution v1.5.0, research.md §12)
...
### UI prototype checkpoint for User Story 3 (constitution v1.6.0, Principle XII)

- [ ] T058b [US3] **Prototype checkpoint — Eduardo approval required.** Build a throwaway React prototype of the Order Budgeting wizard screens (Customer picker, Recipe line editor with live-updating cost breakdown, Future/Pending stock prompt, budget summary with labor/profit/discount) under gitignored `prototypes/orders-budget/`, using mock/static data only — no tests, no Supabase wiring. Present it to Eduardo for explicit approval. **Do not start T059 or any other Phase 5 task until approved.** The prototype is discarded afterward and never merged (research.md §13).
...
- [ ] T074 [P] [US3] Write pgTAP test `supabase/tests/database/129_stock_batch_availability.test.sql` asserting `cost_per_unit_amount` = `package_price` ÷ converted package content; `available_amount` = `remaining_amount` minus reservations from **active** Orders only, so a canceled Order's reservations free their amount and a consolidated Order's are not double-counted (FR-031, spec edge case); that a reservation with a matching `order_stock_consumptions` row is excluded even while its Order is still `awaiting_consolidation`, so a *partially* consolidated Order frees each Batch as that item is confirmed instead of holding the deducted amount and the commitment at once — assert it both ways, with a second reserved item on the same Order still uncounted; and the `is_expired`/`expires_soon` flags, **including a Batch whose `expiration_date` is NULL, where both flags MUST come back `false` rather than NULL** so an undated Material Batch stays eligible without earning the 15-day priority (FR-011, data-model.md) — must fail
...
- [ ] T077 [P] [US3] Create the shared batch-selection scenario set as `tests/fixtures/batchSelection.ts` and its SQL twin `supabase/tests/database/fixtures/batch_selection.sql`, covering: a cheap late-expiring Batch vs. a dearer one expiring in 10 days, an already-expired Batch, two Batches tied on price and expiry, a need larger than all Batches combined, **a Batch with no expiration date that is cheaper than one expiring in 10 days, and an undated Batch tied on price with a dated one** — both T078 and T080 assert against these same cases so the TypeScript and SQL orderings cannot drift (research.md §7). The two undated cases are not optional padding: NULL expiry is the normal case for Materials (FR-011) and is the one input on which a naive TypeScript and a naive SQL implementation diverge in opposite directions, so a fixture set without them cannot do the job it exists for
- [ ] T078 [P] [US3] Write Vitest unit test `tests/unit/costing/selectBatchesForNeed.test.ts` driven by T077's fixtures: expired Batches are never drawn; Batches expiring within 15 days come first, cheapest first within that group; remaining Batches cheapest first; ties broken by expiration date then batch id; greedy draw across several Batches; leftover reported as `shortfallAmount` (FR-022); and an undated (`null` expiration) Batch is never treated as expired nor as expiring soon, so it loses to a dearer Batch expiring within 15 days, beats later-expiring dated Batches on price, and loses a price tie to a dated one (FR-011) — must fail
...
- [ ] T080 [P] [US3] Write pgTAP test `supabase/tests/database/131_reserve_stock_for_order.test.sql` per contracts/rpc-reserve-stock-for-order.md, driven by the same T077 fixtures: identical ordering to T078 — explicitly including the two undated-Batch cases, which must be selected and ordered identically on both sides, since `where is_expired = false` and `order by expires_soon desc` are exactly where SQL's NULL handling would otherwise drop or promote them; idempotent re-run rebuilds the line's reservations; a placeholder is used for the remainder only when one already exists for that Order + item, with `unit_cost_is_estimated = true`; otherwise the shortfall is returned uncovered while the real-Batch reservations are kept; `uncovered_shortfalls` is Order-wide, not line-local (FR-016); `costed_at` is set; and the call errors when the Order is not `quoting` — must fail
...
- [ ] T091 [P] [US3] Write Vitest unit test `tests/unit/costing/computeOrderBreakdown.test.ts` per contracts/costing-engine.md: per-line profit; labor allocated in proportion to line cost and split evenly when every line's cost is 0 (FR-027); discount allocated in proportion to line subtotal, with every line's share 0 when the pre-discount subtotal is 0 rather than `NaN` from a `0/0` (FR-027); `pricePerUnit`; per-line figures summing back to the Order totals; matching quickstart.md's US3 hand-computed example (FR-024–FR-027) — must fail
...
- [ ] T095a [P] [US3] Write RTL test `tests/components/orders/AdvanceToBudgetingAction.test.tsx`: the action calls `transition_order_status` with `p_new_status = 'quoting'` for an Order in Awaiting Quote, surfaces the RPC's rejection inline rather than silently failing, and is not offered on an Order that has already left Awaiting Quote (FR-019/FR-029) — must fail
...
- [ ] T103a [US3] Implement `src/features/orders/AdvanceToBudgetingAction.tsx` satisfying T095a, calling `transition_order_status` with `p_new_status = 'quoting'`. This is the only client path onto the `awaiting_quote → quoting` edge that T085 implements: FR-019 permits Recipe lines only once the Order is in Quoting, and `reserve_stock_for_order` errors outright unless the Order is `quoting`, so without this action an Order created by T103 can never reach the line editor in T104
...
### e2e regression for User Story 3 (constitution v1.5.0)

- [ ] T108a [US3] Write Playwright e2e spec `tests/e2e/orders-budget.spec.ts` covering the budgeting flow end to end: create an Order, add Recipe lines against existing Stock, confirm the cost breakdown updates live and matches the eligible Batches, trigger and resolve a Future/Pending shortfall, and confirm the final price and per-line allocation display; run against the completed Phase 5 implementation and confirm passing (constitution v1.5.0, research.md §12)
...
### UI prototype checkpoint for User Story 4 (constitution v1.6.0, Principle XII)

- [ ] T108b [US4] **Prototype checkpoint — Eduardo approval required.** Build a throwaway React prototype of the Production/Consolidation screens (Future-stock resolution, production-start action, delivery/payment form, consolidation checklist, budget-vs-actual summary, cancel action, status-history timeline) under gitignored `prototypes/orders-production/`, using mock/static data only — no tests, no Supabase wiring. Present it to Eduardo for explicit approval. **Do not start T109 or any other Phase 6 task until approved.** The prototype is discarded afterward and never merged (research.md §13).

### Schema and RPCs for User Story 4

- [ ] T109 [P] [US4] Write pgTAP test `supabase/tests/database/140_order_stock_consumptions.test.sql` asserting RLS, `consumed_amount` > 0, the unique (`order_recipe_line_id`, `stock_batch_id`) constraint that makes FR-034's completeness check a count comparison, and that `assert_parent_order_mutable` rejects UPDATE and DELETE once the parent Order is terminal while still permitting the INSERTs `consolidate_order_stock` makes during `awaiting_consolidation` (FR-037) — must fail
- [ ] T110 [US4] Run `supabase migration new order_stock_consumptions_table` and, in the generated `supabase/migrations/<timestamp>_order_stock_consumptions_table.sql`, create the table per data-model.md with RLS, attaching the same `assert_parent_order_mutable` trigger the Phase 5 child tables use (FR-037); confirm T109 passes
...
- [ ] T113 [P] [US4] Write pgTAP test `supabase/tests/database/142_transition_order_status_v2.test.sql`: `awaiting_production → in_production` is blocked while any reservation references a `future_stock_placeholders` row and succeeds after `resolve_future_stock` (FR-030); `in_production → awaiting_consolidation` requires `p_delivered_at` and `p_payment_method`, accepts `pix`/`cash`/`deferred`, and **writes both onto the `orders` row** — the record FR-032 asks for does not exist if the parameters are only validated and then dropped; `* → canceled` releases reservations first, is reachable from every non-terminal status, and **persists `p_cancel_reason`/`p_cancel_impact_note` onto the `orders` row when given**, while still succeeding when both are omitted since FR-036 makes them optional; any transition attempted from `consolidated` or `canceled` is rejected (FR-037); and T084's v1 edges still behave — must fail
...
- [ ] T117 [P] [US4] Write pgTAP test `supabase/tests/database/144_terminal_order_readonly.test.sql` asserting that once an Order is `consolidated` or `canceled`, UPDATE and DELETE are rejected on `orders`, `order_recipe_lines`, `order_recipe_line_material_overrides`, `order_stock_reservations`, and `order_stock_consumptions` alike (FR-037). This is a **sweep**, not a red-green pair: most of it is already guaranteed by the Phase 5 triggers and by T110, so run it first and record the result. Any assertion that passes straight away is confirmed coverage; only for one that fails do you add the missing guard in `supabase migration new terminal_order_guards` and re-run to green
...
### e2e regression for User Story 4 (constitution v1.5.0)

- [ ] T136a [US4] Write Playwright e2e spec `tests/e2e/orders-production.spec.ts` covering the full lifecycle end to end: resolve a Future/Pending placeholder, advance a budgeted Order through Awaiting Production → In Production → Awaiting Consolidation → Consolidated confirming a partial-package consumption and the real-vs-budgeted comparison, and separately cancel a non-terminal Order and confirm its stock frees up; run against the completed Phase 6 implementation and confirm passing (constitution v1.5.0, research.md §12)
...
- [ ] T139 Run `npm run lint`, `npm run build`, `npm test`, `npm run test:db`, and `npm run test:e2e`; fix any failures (Development Workflow gate, constitution v1.5.0)
...
- **User Story 2 (Phase 4)**: Depends on Foundational only; independent of US1's tables, though both share the nav shell from T019. Blocked at the very start on T041d (prototype approval, Principle XII) before T042 may begin.
- **User Story 3 (Phase 5)**: Depends on Foundational, and on US1 and US2 having created `recipe_size_variants` and `stock_batches`, which its FKs and its `order_line_requirements`/`stock_batch_availability` views reference. Note that `stock_batch_availability` is created here rather than in US2, because `available_amount` is defined against `order_stock_reservations`; US2's shelf UI reads `stock_batches.remaining_amount` directly and stays independently deliverable. Blocked at the very start on T058b (prototype approval, Principle XII) before T059 may begin.
- **User Story 4 (Phase 6)**: Depends on User Story 3 — it extends `transition_order_status` and the reservation machinery, and cannot be built first. Blocked at the very start on T108b (prototype approval, Principle XII) before T109 may begin.
- **Polish (Phase 7)**: Depends on all four user stories.

### Within Each User Story

- Each test task is written and confirmed **failing** before the implementation task that follows it. The one exception is each story's e2e regression task (T041a–c backfilled for US1; T058a for US2; T108a for US3; T136a for US4) — these run last, against the story's completed implementation, and are confirmed **passing**, not failing-first (constitution v1.6.1, Principle I; see the Tests note at the top of this file).
- For US2, US3, and US4 (not US1, which is grandfathered — see its checkpoint note), a UI-prototype-and-approval checkpoint (T041d, T058b, T108b respectively) sits before that story's first schema/implementation task and blocks everything else in the phase until Eduardo approves it (constitution v1.6.0, Principle XII).
...
- Every test task MUST be run and confirmed **failing** before its paired implementation task starts (Constitution Principle I, non-negotiable), except each story's e2e regression task, which is confirmed passing after the story's implementation is done, and T117, which sweeps guards earlier phases already installed and only turns into a red-green cycle for an assertion that actually fails. No implementation task in this list is unpaired.
...
- **Task ID convention for this revision (2026-09-16)**: `T041a`–`T041d`, `T058a`–`T058b`, `T095a`, `T103a`, `T108a`–`T108b`, and `T136a` were added by amending this file in place after constitution v1.5.0/v1.6.0/v1.6.1 landed, to avoid renumbering any task from `T042` onward — several already have GitHub Issues from `/speckit-taskstoissues`, and T001–T041 are already complete. Each suffixed ID is anchored to, and sits immediately after, the nearest preceding original task ID in document order; read the file top to bottom for execution order, not the numeric value of the ID. Run `/speckit-taskstoissues` again to reconcile Issues with these additions via diff-and-patch, not close-all-and-recreate.
