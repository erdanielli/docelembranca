...
- **FR-027**: System MUST display, for the Order as a whole and per Recipe line, the detailed cost breakdown (ingredient cost, material cost, labor allocation, profit amount, discount amount) and the final price to charge. The Order's single labor cost (FR-024) and single discount percentage (FR-026) MUST be allocated across Recipe lines in proportion to each line's own cost, so a per-line — and therefore a per-unit — price can be read directly. Two degenerate cases have no proportion to divide by and MUST be handled explicitly rather than left to produce a divide-by-zero: when every Recipe line's cost is zero, the labor cost MUST be split evenly across the lines instead of being dropped from the total; and when the Order's pre-discount subtotal is zero, there is nothing to discount, so every line's discount share MUST be zero.
...
- **FR-037**: A Canceled or Consolidated Order MUST be archived as read-only: no further status transitions, and no edits to its Recipe lines, labor cost, discount, material overrides, stock commitments, or confirmed consumption records. The consumption records are covered for the same reason as the rest: they are the "actual" half of the budgeted-vs-actual comparison in FR-035, so an Order that could still have them edited after Consolidation would not have a fixed financial outcome at all.
...
- A stock commitment counts against a Batch's availability only while its Order is still active (between Budgeting and Consolidation) **and its actual consumption has not yet been confirmed**; canceled Orders release theirs immediately, and a confirmed consumption has already been deducted from the Batch's remaining content, so continuing to count the commitment on top of it would reserve the same stock twice. This matters during Consolidation in particular, which is confirmed one item at a time: the items already confirmed free up for other Orders straight away rather than waiting for the last one.
- Labor cost and the order-level discount are allocated to Recipe lines in proportion to each line's cost (FR-027). The one departure is the degenerate case FR-027 names: when every line's cost is zero there is no proportion to divide by, so labor is split evenly rather than silently dropped. That is a fallback for an impossible-to-allocate total, not a second allocation basis — no alternative basis (per unit, per line count) is offered as a choice.
